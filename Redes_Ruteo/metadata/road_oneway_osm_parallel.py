...
# -*- coding: utf-8 -*-
"""
Parallel extractor for OSM oneway tags.
Output: metadata/road_oneway.geojson (LineString features with osm_id + oneway)
ENV similar to others.
"""
...
def ways_to_features(data:Dict[str,Any])->List[Dict[str,Any]]:
    ...
    for el in data.get("elements",[]) or []:
        ...
        tags=el.get("tags",{}) or {}
        oneway_raw = (tags.get("oneway") or "").strip().lower()
        oneway = True if oneway_raw in ("yes","true","1") else False if oneway_raw in ("no","false","0") else None
        feats.append({
            "type":"Feature",
            "geometry":{"type":"LineString","coordinates":coords},
            "properties":{"osm_id": int(el["id"]), "oneway": oneway}
        })
    return feats
...
def main():
    ...
    print(f"[ONEWAY] Parallel tiles {ROWS}x{COLS} (workers={max_workers})")
    feats_all=[]
